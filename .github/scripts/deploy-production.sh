...
# ==============================================================================
# This script is executed on the production server via SSH to deploy the application.
...
while [ $ELAPSED -lt $MAX_WAIT ]; do
  ...
  if [ -z "$UNHEALTHY" ] && [ -z "$STARTING" ]; then
    ...
    docker compose -f docker-compose.yml -f compose.production.yml exec -T redis \
