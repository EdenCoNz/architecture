...
from apps.core.database import DatabaseHealthCheck, check_database_connection, get_database_status
...
class TestAcceptanceCriteria1:
    ...
    def test_database_connects_successfully_on_startup(self):
        ...
        assert result["status"] == "healthy", "Database should be healthy when server starts"
        assert result["database"] == "connected", "Database should be connected"

        # Should have reasonable response time
        assert "response_time_ms" in result, "Should report response time"
        assert result["response_time_ms"] > 0, "Response time should be positive"
        assert result["response_time_ms"] < 5000, "Response time should be less than 5 seconds"
    ...
    def test_startup_provides_connection_feedback(self, capsys):
        ...
        assert "✓ Database connected" in captured.out, "Should print success message on startup"
    ...
    def test_check_database_command_shows_connection_success(self):
        ...
        # Should show success
        assert (
            "✓ Database connection successful" in output
        ), "Command should show connection success"

        # Should show connection details
        assert "Connection Details:" in output, "Should display connection details"
        assert "Database:" in output, "Should show database name"
        assert "Host:" in output, "Should show database host"
...
class TestAcceptanceCriteria3:
    ...
    def test_connection_failure_shows_clear_error_message(self):
        ...
        with patch("django.db.connection.cursor") as mock_cursor:
            ...
            assert "connect" in result["error"].lower(), "Error should mention connection problem"
            assert (
                "PostgreSQL" in result["error"] or "database" in result["error"].lower()
    ...
    def test_graceful_degradation_with_warning(self, capsys):
        ...
        with patch("django.db.connection.cursor") as mock_cursor:
            ...
            assert result is False, "Should return False when database unavailable"

            captured = capsys.readouterr()

            # Should print warning to stderr
            assert "WARNING" in captured.err, "Should print warning"
            assert "Database connection failed" in captured.err, "Should describe the problem"
            assert "check_database" in captured.err, "Should suggest diagnostic command"
    ...
    def test_graceful_degradation_with_failure(self, capsys):
        ...
        with patch("django.db.connection.cursor") as mock_cursor:
            ...
            assert "FATAL" in captured.err, "Should print FATAL error"
            assert "Database connection failed" in captured.err, "Should describe the problem"
    ...
    def test_management_command_provides_troubleshooting(self):
        ...
        with patch("django.db.connection.cursor") as mock_cursor:
            ...
            assert "Troubleshooting:" in output, "Should provide troubleshooting section"
            assert (
                "PostgreSQL is running" in output
            ), "Should suggest checking if PostgreSQL is running"
            assert (
                "createdb" in output or "database" in output.lower()
...
class TestAcceptanceCriteria4:
    ...
    def test_environment_example_file_exists(self):
        ...
        assert "DB_NAME" in content, ".env.example should include DB_NAME"
        assert "DB_USER" in content, ".env.example should include DB_USER"
        assert "DB_PASSWORD" in content, ".env.example should include DB_PASSWORD"
        assert "DB_HOST" in content, ".env.example should include DB_HOST"
        assert "DB_PORT" in content, ".env.example should include DB_PORT"
    ...
    def test_health_check_does_not_expose_password(self):
        ...
        assert "password" not in result_str, "Health check should not expose password"

        # Connection info should not include password
        if "connection_info" in result:
            conn_info = result["connection_info"]
            assert "password" not in conn_info, "Connection info should not include password field"
            assert "PASSWORD" not in conn_info, "Connection info should not include PASSWORD field"
    ...
    def test_database_status_does_not_expose_credentials(self):
        ...
        assert "password" not in status_str, "Database status should not expose password"
    ...
    def test_management_command_does_not_expose_credentials(self):
        ...
        assert "password" not in output, "Command output should not show password"
...
class TestOverallDataPersistenceLayer:
    ...
    def test_can_connect_to_database(self):
        ...
        assert (
            check_database_connection(verbose=False) is True
    ...
    def test_can_execute_queries(self):
        ...
        assert result == (1,), "Should be able to execute queries"
